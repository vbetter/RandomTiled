	...
	public class PatternInspector : Editor {
		...
		public override void OnInspectorGUI() {
			...
			if (notAPrefab) {
				...
				int tempGridZ;
				tempGridZ = EditorGUILayout.IntField ("Grid Z", pattern.gridZ);
				if (tempGridZ != pattern.gridZ) {
					//Check if there are children that lie outside the grid area
					if (TilesOutsideModifiedGrid (tempGridZ) == false) {
		...
		private void ValidateData() {
			if (_previewRenderUtility == null) {
				...
				_previewRenderUtility.m_Camera.transform.position = new Vector3 (18.7f, 17.9f, -19.3f);
				_previewRenderUtility.m_Camera.transform.rotation = Quaternion.Euler (35, 315, 0);
				...
				_previewRenderUtility.m_Camera.farClipPlane = 1000f;
			}
		}
		...
		private void OnSceneGUI() {
			...
			if (inEditMode) {
				...
				if (IsMouseOnLayer ( mouseHitPosition, pattern.gridZ)) {
		...
		* */
		...
		private bool UpdateHitPosition() {
			Plane gridPlane = new Plane (Vector3.up, pattern.transform.position);
		...
		private void RecalculateMarkerPosition() {
			...
			DrawWireCube(new Vector3(tilePosition.x * PatternSettings.tiledSize, pattern.transform.position.y, (tilePosition.y * PatternSettings.tiledSize)), PatternSettings.tiledSize);
		}
		...
		private Vector2 GetTilePosition(Vector3 fromPosition) {
			Vector3 pos = new Vector3 (fromPosition.x / PatternSettings.tiledSize, fromPosition.y, fromPosition.z / PatternSettings.tiledSize);
			...
			if (pos.z < -pattern.gridZ * 0.5f) {
				if (Mathf.Approximately (pattern.zOffset, 0)) {
					pos.z = Mathf.FloorToInt (-pattern.gridZ * 0.5f) + 1;
				} else {
					pos.z = Mathf.FloorToInt (-pattern.gridZ * 0.5f);
				}
			} else if (pos.z > (pattern.gridZ * 0.5f) - (pattern.zOffset/PatternSettings.tiledSize)) {
				if (Mathf.Approximately (pattern.zOffset, 0)) {
					pos.z = Mathf.FloorToInt (pattern.gridZ * 0.5f);
				} else {
					pos.z = Mathf.FloorToInt (pattern.gridZ * 0.5f)-1;
		...
		private bool IsMouseOnLayer(Vector3 testPosition, int testGridZ) {
			Vector3 pos = new Vector3 (testPosition.x / PatternSettings.tiledSize, testPosition.y, testPosition.z / PatternSettings.tiledSize);
			pos.x = Mathf.RoundToInt (pos.x);
			pos.z = Mathf.RoundToInt (pos.z);

			return !(pos.x < -PatternSettings.gridX * 0.5f || pos.x > PatternSettings.gridX * 0.5f || pos.z < -testGridZ * 0.5f || pos.z > testGridZ * 0.5f);
		}
		...
		private void Draw() {
			...
			if (tile != null && destroyTile == false) {
				...
				tile.transform.localPosition = new Vector3 (tilePosition.x * PatternSettings.tiledSize, pattern.transform.position.y,
					tilePosition.y * PatternSettings.tiledSize);

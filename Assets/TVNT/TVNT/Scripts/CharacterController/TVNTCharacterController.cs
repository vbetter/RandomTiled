...
namespace TVNT {
	public class TVNTCharacterController : MonoBehaviour {
		...
		protected void AttemptMove() {
			...
			if (Physics.Linecast (parent.parent.TransformPoint(start + barrierOffset), parent.parent.TransformPoint(end + barrierOffset), out hitInfo, barrierLayerMask) == false) {
				...
				if (canSmash && hitInfo.transform.tag == "Smashable") {
					idle = false;
					hitInfo.transform.parent.parent.GetComponent<ISmashable> ().Smash (0.175f);
					horizontal = 0;
					vertical = 0;
					myAnimator.SetTrigger("Jump");
					myAnimator.speed = 2;
		...
		public void PlayIdleAnimation() {
			if (jump == false) { //Set the idle animation only if the movement style of the character is walk based; jump transitions into idle by itself
		...
		protected virtual void LifeLost(int currentLives) {
